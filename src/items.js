...
const GEAR = [
  ...
  /*
   * MILK, which bucket.js has been waiting for.
   *
   * That file already has the whole behaviour written and guarded behind
   * `try { MILK_BUCKET = itemId('milk_bucket') } catch {}`, with a note saying
   * the missing piece is one row here and that items.js belonged to somebody
   * else at the time. This is the row. Nothing in bucket.js changes: the
   * lookup stops throwing and the branch lights up.
   *
   * It belongs in the potion wave rather than anywhere else because milk is
   * the CURE -- MilkBucketItem is literally `entity.removeAllEffects()` -- and
   * a world that can now poison you should be able to un-poison you.
   */
  { key: 'milk_bucket', name: 'Milk Bucket', stack: 1 },
  /*
   * The empty bottle a drunk potion hands back. `usingConvertsTo(GLASS_BOTTLE)`
   * in vanilla's Items.java, which is the `use_remainder` component.
   * stacksTo is the default 64 -- it is the POTION that is stacksTo(1).
   */
...
/*
 * POTIONS, and every one of them is 58 rows generated by potions.js rather
 * than written here.
 *
 * They arrive as a list because the thing that decides WHICH potions exist is
 * the vanilla table plus the "does anything in this world consume that
 * effect" test, and both of those live next to each other in potions.js. This
 * file's job is the one it does for every other item: give it a number, give
 * it the standard shape, and put it in the registry.
 *
 * They are UNPLACEABLE in the sense the list above means -- `places` is 0 --
 * but they are not in UNPLACEABLE, because that list is specifically for
 * things that WOULD be blocks in Minecraft and cannot be here yet. A potion
 * is not a block in Minecraft either. The `places: 0` default in the assembly
 * below is all it needs, which is the same route a stick and a pickaxe take.
 */
const NON_BLOCK = [...MATERIALS, ...UNPLACEABLE, ...TOOLS, ...ARMOR, ...GEAR, ...POTION_ITEMS]
  .map((def, i) => ({
    id: ITEM_BASE + i,
    places: 0,
...
export const ITEM_TEXTURES = ITEMS
  .filter(i => i.texture)
  .map(({ texture, from, layers }) => ({ name: texture, from, layers }))
...
const DROP_RULES = [
  ...
  /*
   * Signs, all twenty of them, and `self` covers every one for the same reason
   * it covers the torches: blocks.js points the nineteen variants' `drops` at the
