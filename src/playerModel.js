...
/*
 * keepMaterialLive MOVED to entityLight.js.
 *
 * It was written here only because entityLight.js was being rewritten the day
 * it was needed, and its own comment said where it belonged. It is now called
 * by `trackEntityLight` on every entity material, so nothing has to remember
 * to call it. Re-exported rather than deleted because heldItem.js and
 * itemEntity.js import it from this file and neither is this change's to edit.
 */
export { keepMaterialLive } from './entityLight.js'
...
export function createSkinMaterial(noa, url, name = 'skin', onError = null) {
   ...
   */
  // trackEntityLight calls keepMaterialLive itself now; without it every
  // number entityLight.js writes is correct in JavaScript and absent on
  // screen. See the comment on it in entityLight.js.
