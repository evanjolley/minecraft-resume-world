...
distributed, and right now the only place carrying it is `public/textures/NOTICE.txt` --
which also records that `grass_side.png` is our derivative. That file ships, but nothing in
the page points a visitor at it, so an in-game credits surface is a prerequisite for going
live; see `docs/FUTURE.md`.
